...
func (h *Handler) updateCategories(rw http.ResponseWriter, r *http.Request) {
	...
	const getCategory = `SELECT * FROM categories WHERE id = $1`
	var category Category
	if err := h.db.Get(&category, getCategory, id); err != nil {
		http.Error(rw, "Category not found", http.StatusInternalServerError)
		return
	}

	if err := r.ParseForm(); err != nil {
		http.Error(rw, "Invalid form data", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		http.Error(rw, "Invalid status value", http.StatusInternalServerError)
		return
	}
	...
	if ok, err := res.RowsAffected(); err != nil || ok == 0 {
		http.Error(rw, "Failed to update category", http.StatusInternalServerError)
		return
	}
